...
public class BookReport {
    public static void exportToPDF(DefaultTableModel tableModel) {
        ...
        try {
            ...
            String filename = "BookReport_" + timestamp + ".pdf";
            PdfWriter.getInstance(document, new FileOutputStream(filename));
            document.open();

            // Judul
            Font titleFont = new Font(Font.FontFamily.HELVETICA, 18, Font.BOLD);
            Paragraph title = new Paragraph("Book Report", titleFont);
            ...
            // Tabel
            PdfPTable pdfTable = new PdfPTable(5); // 5 kolom sesuai tabel
            pdfTable.setWidthPercentage(100);
            pdfTable.setWidths(new float[]{1, 2, 2, 2, 1}); // Lebar kolom disesuaikan

            Font headerFont = new Font(Font.FontFamily.HELVETICA, 12, Font.BOLD);
            String[] headers = {"ID", "Title", "Author", "Category", "Stock"};
            ...
            for (int row = 0; row < tableModel.getRowCount(); row++) {
                for (int col = 0; col < tableModel.getColumnCount(); col++) {
                    ...
                    cell.setHorizontalAlignment(col == 0 || col == 4 ? Element.ALIGN_CENTER : Element.ALIGN_LEFT); // Rata tengah untuk ID dan Stock
                    pdfTable.addCell(cell);
                }
            }
